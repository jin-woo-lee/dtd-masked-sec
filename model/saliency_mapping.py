...
class ActivationStoringNet(nn.Module):
    ...
    def update_module_list(self, module_list):
...
class DTD(nn.Module):
    def __init__(self, lowest=0., highest=1., device='cuda:0'):
    ...
    def forward(self, module_stack, y, class_num, model_archi):
        R = torch.eye(class_num)[torch.max(y, 1)[1]].to(self.device).float()

        # z^{B}-Rule: v_i = x_i - l_i * 1_{w_{ij}>0} - h_i * 1_{w_{ij}<0}
    ...
    def backprop_conv(self, activation, module, R):
        ...
        #++++++++++++++++++++++++++++++ 
        op = activation.size(2) - ((R.size(2) - 1) * stride[0] - 2 * padding[0] + kernel[0])
        if stride[0]==2 and op < 1:
            output_padding = (0,1)
        elif stride[0]==2 and op == 1:
            output_padding = (1,1)
        else:
            output_padding = op
        #++++++++++++++++++++++++++++++ 
        ...
        #print("-"*10)
        #print(stride, padding, kernel, output_padding)
